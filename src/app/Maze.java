...
public class Maze {
	 ...
	 * @throws IllegalArgumentException if the values provided for {@code rows} or
	 *                                  {@code cols} are less than 2.
	 */
	...
	private void generate(int row, int col) {
		...
		/*
		 * Clone a new list from the {@code DELTAS} array and shuffle it, creating a
		 * random order of directions to visit. This step is responsible for generating
		 * unique maze designs.
		 * 
		 * NOTE: It's necessary to clone a NEW list, since it appears that {@code
		 * Arrays.asList()} may mutate the original array. Otherwise, the maze is
		 * generated with isolated vertices (cells with walls on all four sides and no
		 * connected edges).
		 */
		...
		for (int[] delta : directions) {
			...
			/*
			 * Adds a delta (difference) value to the current row and column. The delta may
			 * be either -1, 0, or 1, which will correspond to the vertex directly adjacent
			 * to the current vertex, either N, S, E, or W.
			 * 
			 * For example, to visit the vertex north of the current vertex, shift up from
			 * the current row by 1 (delta 1) but stay in the same column (delta 0). The
			 * value of that vertex will ultimately be equal to that of the current vertex +
			...
			/*
			 * Check if the calculated adjacent vertex is in bounds and that the program has
			 * not yet visited it. The {@code visited} check also serves as the recursion's
			 * base case.
			 */
	 ...
	 * Without verifying the boundaries of the maze, it's possible that the adjacent
	 * vertex is "outside" of the maze and shouldn't exist.
	 *
	 * @param row the row index to check
	 * @param col the column index to check
	 * @return {@code true} if (row, col) is a valid cell in the grid; {@code false}
	 *         otherwise
	 */
